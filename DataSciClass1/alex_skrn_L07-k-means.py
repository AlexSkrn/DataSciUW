...
# -*- coding: utf-8 -*-
"""
This files contains Lesson 7 Assignment.
K-Means.
...
#import logging
#
#logging.basicConfig(filename="L07_log.log",
#                    filemode='w',
...
#    my_data["age_z"] = standard_scaler.transform(age_df)
#    print("z-normalized 'age_z' column's mean and std are now {:.3f} and {:.3f}"
#          .format(my_data.age_z.mean(),
#                  my_data.age_z.std()
#                  )
#          )
#
...
#    """)
#    hr_wk_df = my_data[['hours-per-week']]
#    standard_scaler = preprocessing.StandardScaler().fit(hr_wk_df)
#    my_data["hours-per-week_z"] = standard_scaler.transform(hr_wk_df)
#    print("""z-normalized 'hours-per-week_z' column's mean and std are now {:.3f}
#    and {:.3f}"""
#          .format(my_data.loc[:, "hours-per-week_z"].mean(),
#                  my_data.loc[:, "hours-per-week_z"].std()
#                  )
#          )
...
#          """)
    ...
    print("""Trying to make a scatter plot of age and hour-per-week
          ...
          """)
    age_hours = my_data[['age', 'hours-per-week']].copy()
    age_hours_std = stats.zscore(age_hours[['age', 'hours-per-week']])
    kmeans = KMeans(n_clusters = 4).fit(age_hours_std)
    y_kmeans = kmeans.predict(age_hours_std)
    plt.scatter(my_data.loc[:, 'age'],
                my_data.loc[:, 'hours-per-week'],
                c=y_kmeans)
    plt.xlabel('age')
    plt.ylabel('hours-per-week')
    plt.title('Age vs hours of work per week')
    plt.show()
    ...
    print("Trying to apply k-means to several binary attributes:\n")
    df_cp = my_data.copy()

    # Create dummies for several attributes
...
#    print(df_cp.columns)
    ...
    clmns = ['marital_married', 'marital_unmarried', 'wrk_cla_gov',
             'wrk_cla_private', 'sex_ Female', 'sex_ Male']
    df_tr_std = stats.zscore(df_cp[clmns])
    ...
    df_cp['clusters'] = labels

    # Add the column into our list
    clmns.extend(['clusters'])

    # Analyze the clusters
    print(df_cp[clmns].groupby(['clusters']).mean())
